...
def get_prompt(config: Dict, category: str, custom_prompt_dir: str) -> str:
    """Retrieves the prompt for a given category, handling defaults."""
    prompt = ""
    ...
    if not prompt:
        logging.warning(
            f"No prompt found for category '{category}'.  Using a generic default."
        )
...
def create_backup(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        console.print(f"[green]Backup created: {backup_path}[/green]")  # Show backup path
        return backup_path
    except Exception as e:
...
def restore_backup(file_path: str, backup_path: str) -> None:
    ...
    try:
        ...
        console.print(f"[yellow]File restored from: {backup_path}[/yellow]")  # Indicate restoration
    except FileNotFoundError:
        ...
        if isinstance(node, (ast.FunctionDef, ast.ClassDef, ast.AsyncFunctionDef)):
             ...
             for category in categories:
                ...
                if "{code}" not in prompt:
                    ...
                    progress.update(
                        improve_task_id,
                        advance=1 / len(categories),
                        fields={"status": "Prompt Error"},
                    )
                ...
                for attempt in range(MAX_LLM_RETRIES):
                    retry_counts[category] = attempt + 1  # Update retry count
                    progress.update(
                        improve_task_id,
                        description=f"[blue]Improving {category} (attempt {attempt+1}/{MAX_LLM_RETRIES})[/blue]",
                    ...
                    try:
                        ...
                        improved_code_snippet = extract_code_from_response(
                            response.choices[0].message.content
                        )
                        if validate_python_syntax(improved_code_snippet):
                            # Replace original code snippet
                            original_lines = updated_code.splitlines()
                            updated_lines = []
                            if start_line > 1:
                                updated_lines.extend(original_lines[:start_line - 1])
                            updated_lines.extend(improved_code_snippet.splitlines())
                            if end_line is not None:
                                updated_lines.extend(original_lines[end_line:])
                            updated_code = "\n".join(updated_lines)
                            improvements_by_category[category] = improved_code_snippet
                            success = True
                            break
                        else:
                            logging.warning(
                                f"Syntax error in LLM response for {category} (attempt {attempt+1}/{MAX_LLM_RETRIES}). Retrying..."
                            )
                    except APITimeoutError:
                        logging.warning(
                            f"Timeout in category {category} (attempt {attempt+1}/{MAX_LLM_RETRIES})"
                        )
                    except Exception as e:
                        logging.exception(
                            f"LLM error in category {category} (attempt {attempt+1}/{MAX_LLM_RETRIES}): {e}"
                        )
                ...
                progress.update(
                    improve_task_id, advance=1 / len(categories), fields={"status": "Completed"}
                )
