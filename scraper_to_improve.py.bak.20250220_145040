    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt == MAX_RETRIES - 1:
                logging.error(
                    "Failed to make request to %s after %d attempts: %s",
                    url,
                    MAX_RETRIES,
                    e,
                )
            ...
            console.print(
                "[yellow]Request failed: %s. Retrying in %s seconds... (Attempt %d/%d)[/yellow]"
                % (e, wait_time, attempt + 1, MAX_RETRIES)
            )
            logging.warning(
                "Request failed: %s. Retrying in %s seconds... (Attempt %d/%d)",
                e,
                wait_time,
                attempt + 1,
                MAX_RETRIES,
...
def get_rate_limit_status(token):
    ...
    try:
        ...
        console.print(
            "[red]Unexpected error getting rate limit status: %s[/red]" % e
        )
...
def get_file_content_and_stats(token, repo_full_name, file_path):
    ...
    try:
        ...
        logging.error(
            "Error getting file stats for %s/%s: %s",
            repo_full_name,
            file_path,
            e,
        )
    ...
    try:
        ...
        try:
            repo_info = make_github_request(
                "%s/repos/%s" % (GITHUB_API_URL, repo_full_name), headers
            )
        except GitHubAPIError as e:
            logging.error(
                "Failed to fetch repository info for %s: %s", repo_full_name, e
            )
        ...
        if (
            ...
            logging.warning(
                "Repository info for %s is invalid or missing 'default_branch'.  Repo info: %s",
                repo_full_name,
                repo_info,
            )
...
def main():
    ...
    try:
        ...
        console.print(
            ...
            % (
                rate_limit_status["core"]["remaining"],
                rate_limit_status["search"]["remaining"],
            )
        )
        ...
        if rate_limit_status["core"]["remaining"] == 0:
            ...
            wait_time = max(0, wait_time)
            console.print(
                "[yellow]Core rate limit is 0. Waiting until %s (%s seconds)[/yellow]"
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    console.print(
                        "[red]Error processing a repository: %s[/red]" % e
                    )
